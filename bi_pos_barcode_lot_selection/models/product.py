...
class ProductLot(models.Model):
    ...
    def _compute_avail_locations(self):
        for rec in self:
            rec.avail_locations = []
            locations = rec.quant_ids.filtered(lambda x: x.quantity > 0 and x.location_id.usage == 'internal').mapped(
                'location_id')
            rec.avail_locations = [(6, 0, locations.ids)]
            rec.quant_text = ''
            aa = dict(zip(
                (rec.quant_ids.filtered(lambda x: x.quantity > 0 and x.location_id.usage == 'internal').mapped(
                    'location_id.id')),
                (rec.quant_ids.filtered(lambda x: x.quantity > 0 and x.location_id.usage == 'internal').mapped(
                    'quantity'))
            ))
...
class Product(models.Model):
    ...
    barcode_ids = fields.One2many('stock.production.lot', 'product_id',
                                  string='Barcodes Lots', domain=[("product_qty", ">", 0)])
